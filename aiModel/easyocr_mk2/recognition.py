...
def recognizer_predict(model, converter, test_loader, batch_max_length,\
    ...
    with torch.no_grad():
        for image_tensors in test_loader:
            ...
            # 디코더 종류에 따른 변수 준비
            preds_str = None
            preds_str_1st, preds_str_2nd, preds_str_3rd, preds_str_4th, preds_str_5th = [None]*5
            ...
            # [수정] 'greedy_best2' 라는 새로운 디코더 로직 추가
            elif decoder == 'greedy_best2':
                _, top2_indices = torch.topk(preds_prob, 2, dim=2)
                preds_index_1st = top2_indices[:, :, 0].view(-1)
                preds_index_2nd = top2_indices[:, :, 1].view(-1)
                preds_str_1st = converter.decode_greedy(preds_index_1st.data.cpu().detach().numpy(), preds_size.data)
                preds_str_2nd = converter.decode_greedy(preds_index_2nd.data.cpu().detach().numpy(), preds_size.data)

                        # [추가] greedy_best5 로직
            elif decoder == 'greedy_best5':
                _, top5_indices = torch.topk(preds_prob, 5, dim=2)
                preds_index_1st = top5_indices[:, :, 0].view(-1)
                preds_index_2nd = top5_indices[:, :, 1].view(-1)
                preds_index_3rd = top5_indices[:, :, 2].view(-1)
                preds_index_4th = top5_indices[:, :, 3].view(-1)
                preds_index_5th = top5_indices[:, :, 4].view(-1)
                
                preds_str_1st = converter.decode_greedy(preds_index_1st.data.cpu().detach().numpy(), preds_size.data)
                preds_str_2nd = converter.decode_greedy(preds_index_2nd.data.cpu().detach().numpy(), preds_size.data)
                preds_str_3rd = converter.decode_greedy(preds_index_3rd.data.cpu().detach().numpy(), preds_size.data)
                preds_str_4th = converter.decode_greedy(preds_index_4th.data.cpu().detach().numpy(), preds_size.data)
                preds_str_5th = converter.decode_greedy(preds_index_5th.data.cpu().detach().numpy(), preds_size.data)
            ...
            if decoder == 'greedy_best2':
                for pred1, pred2, pred_max_prob in zip(preds_str_1st, preds_str_2nd, preds_max_prob):
                    confidence_score = custom_mean(pred_max_prob)
                    concatenated_pred =  pred1 + " || " + pred2
                    result.append([concatenated_pred, confidence_score])
            
                        # [추가] greedy_best5 결과 합치기
            elif decoder == 'greedy_best5':
                zipped = zip(preds_str_1st, preds_str_2nd, preds_str_3rd, preds_str_4th, preds_str_5th, preds_max_prob)
                for pred1, pred2, pred3, pred4, pred5, pred_max_prob in zipped:
                    confidence_score = custom_mean(pred_max_prob)
                    concatenated_pred = pred1 + " || " + pred2 + " || " + pred3 + " || " + pred4 + " || " + pred5
                    result.append([concatenated_pred, confidence_score])
            else:
